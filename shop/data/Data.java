...
public class Data {
  ...
  static public RerunnableCommand newUndoCmd(Inventory inventory) {
	    if (!(inventory instanceof InventorySet))
	        throw new IllegalArgumentException();
	    InventorySet inventorySet = (InventorySet) inventory;
	    if(inventorySet.getHistory()==null) {
	    	return CommandHistoryFactory.newCommandHistory().getUndo();
	    }
	    return inventorySet.getHistory().getUndo();
  }
  ...
  static public RerunnableCommand newRedoCmd(Inventory inventory) {
	    ...
	    InventorySet inventorySet = (InventorySet) inventory;
	    if(inventorySet.getHistory()==null) {
	    	return CommandHistoryFactory.newCommandHistory().getRedo();
	    }
	    return inventorySet.getHistory().getRedo();
  }
}  
